...
async function generatePDFReport() {
  ...
  if (_webcamOffForPdf) {
    ...
    // Grid: 3 cols × 3 rows = 9 cells. We have 8 stats so the last
    // row's third column will be empty — visually balanced for a
    // grid-driven layout, and leaves room for the future addition
    // of another anomaly category.
    const cols = 3,
      rows = 3;
  ...
  if (
    ...
    // FIX (May 23): show a proper status badge for BOTH the AI-success
    // and offline-fallback cases, matching the on-screen card. Previously
    // only the fallback case had a note; the AI-success case had no
    // indication that the feedback was actually Gemini-generated.
    // The badge is a small filled rounded rectangle right under the
    // intro, with the same colors used in the in-page card.
    const isFallback = !!data.aiFeedback.fallback;
    const badgeText = isFallback
      ? "Offline fallback (AI was unavailable)"
      : "AI-generated by Gemini";
    const badgeRgb = isFallback ? [146, 64, 14] : [37, 99, 235]; // amber-700 vs sinov-indigo
    const badgeBgRgb = isFallback ? [254, 243, 199] : [219, 234, 254];
    const badgePadX = 6;
    const badgePadY = 3;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    ...
    doc.roundedRect(margin, y, badgeW, badgeH, 2.5, 2.5, "F");
    doc.setTextColor(badgeRgb[0], badgeRgb[1], badgeRgb[2]);
    doc.text(badgeText, margin + badgePadX, y + badgeH - badgePadY - 0.5);
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    y += badgeH + 4;
